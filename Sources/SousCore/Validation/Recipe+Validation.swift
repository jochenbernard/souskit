extension Recipe {
    ...
    /// reference data or other files. Four rules apply: the header states a yield of each
    /// dimension at most once, counting `servings` and every `yield` entry together; no two
    /// group headings state one name; every reference names a group; and no groups consume each
    /// other's intermediates in a loop.
    ///
